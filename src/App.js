...
class App extends Component {
  state = {searchLanguage: '', searchDifficulty: ''}

  changeLanguage = language => {
    this.setState({searchLanguage: language})
  }

  changeDifficulty = level => {
    this.setState({searchDifficulty: level})
  }

  render() {
    const {searchLanguage, searchDifficulty} = this.state
    return (
      <div className="main-bg-container">
        ...
        <div className="card-container">
          <Filters
            onGetSelectedFilters={searchDifficulty}
            languageValue={searchLanguage}
            difficultyLevels={levelsData}
            changeDifficulty={this.changeDifficulty}
            changeLanguage={this.changeLanguage}
            languageLevels={categoryData}
          />
        </div>
        <div className="card-container">
          <InterviewQuestionsApp
            getDifficulty={searchDifficulty}
            getLanguage={searchLanguage}
            interviewQuestionsData={questionsData}
          />
        </div>
      </div>
    )
